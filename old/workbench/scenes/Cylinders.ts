...
export default function () {
  ...
  scenarios.forEach(([lightDirection, title]) => {
    addCylinderScene(lightDirection, title);
  });
}

function addCylinderScene(lightDirection: Vector3, title: string) {
  ...
  const cylinder = Cylinder({ id: title, strokeWidth: 0, height: 100 });
  const scene: Scene = {
    ...getLighting("moonlit"),
    shapes: [
      getEnvironment(),
